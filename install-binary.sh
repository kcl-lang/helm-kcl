...
: "${HELM_PLUGIN_DIR:="$HELM_HOME/plugins/helm-kcl"}"
...
getDownloadURL() {
  ...
  if [ "$SCRIPT_MODE" = "install" ] && [ -n "$version" ]; then
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/download/$version/helm-kcl-$OS-$ARCH.tgz"
  else
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/latest/download/helm-kcl-$OS-$ARCH.tgz"
  fi
}
...
rmTempDir() {
  if [ -d "${HELM_TMP:-/tmp/helm-kcl-tmp}" ]; then
    rm -rf "${HELM_TMP:-/tmp/helm-kcl-tmp}"
  fi
}
